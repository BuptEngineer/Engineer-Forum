...
import com.qust.engineer.dao.UserDao;
...
public class UserService {
	...
	public void regist(User user){
		 ...
		 * 3.发送激活邮件
		 */
		//1.status=0，activationCode
		user.setU_status(false);//表示未激活
		user.setU_actnCode(CommonUtils.uuid()+CommonUtils.uuid());
		//2.添加
		try {
			userDao.add(user);
		} catch (SQLException e) {
		...
		String to = user.getU_email();//收件人
		String subject = pops.getProperty("subject");//邮件的主题
		//MessageFormat.format方法会把第一个参数的占位符{0}，用第二个参数填充
		String content = MessageFormat.format(pops.getProperty("content"), user.getU_actnCode());
