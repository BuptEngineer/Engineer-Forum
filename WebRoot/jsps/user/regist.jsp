 ...
 <div class="whole">
  <form id="submitBtn" action="<c:url value='/UserServlet'/>" method="post"><!--可让用户输入的表单-->
    ...
    <div id="email-element">
         ...
         <input id="email" class="input" type="text" name="u_email" value="${form.u_email }" placeholder="请输入您常用的邮箱地址"/>
		 <span class="spanError" id="emailError">${errors.email }</span>
	 </div>
     <div id="uname-element">
	     <label for="uname">昵称:</label><!--为input元素定义标签-->
         <input id="username" class="input" type="text" name="u_name" value="${form.u_name }" placeholder="起一个响亮的名字吧"/><!--规定用户可输入数据的输入字段-->
	 ...
	 <div id="upass-element">
         ...
         <input id="password" class="input" type="password" name="u_pwd" value="${form.u_pwd }" placeholder="6-20位英文字母、数字或字符"/>
	 ...
	 <div id="repass-element">
         ...
         <input id="confirmpass" class="input" type="password" name="confirmpass" value="${form.confirmpass }" placeholder="请再次输入密码"/>
		 <span class="spanError" id="confirmpassError">${errors.confirmpass }</span>
	 </div>
       <div class="sex-element">
	     <label>性别：</label>
		     <input  class="gender" id="male" type="radio" name="u_sex" value="男"checked="checked"></input><!--checked="checked"是默认第一个选项。-->
		 <label class="gender" for="male">男</label>
		     <input class="gender" id="female" type="radio" name="u_sex" value="女"></input>
	 ...
	 <a href="javascript:_change()">看不清，换一张</a>
	  ...
	  <div id="btn-element">
		<input  id="submitButton" class="btn-success" type="submit" value="注册"></input><!--提交表单时会发生submit函数。-->
